...
describe('Deposits with changing interest recipient', function () {
	...
	it('Two years later, Alice makes Bob interest recipient', async () => {
	...
	it('1 year later, Alice makes Charlie interest recipient', async () => {
		const { time_error } = await this.network.timetravel({shift: '360d'})
		expect(time_error).to.be.undefined

		const new_stable_amount = Math.floor(this.deposit_tokens2 * (1 + this.interest_rate) ** 3.5)
		const stable_delta = new_stable_amount - this.deposit_stable_tokens
		...
		const { unit, error } = await this.alice.triggerAaWithData({
			...
			data: {
				id: this.id,
				change_interest_recipient: 1,
				interest_recipient: this.charlieAddress,
			},
			spend_unconfirmed: 'all',
		})
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		this.deposit_stable_tokens = new_stable_amount
		this.bob_interest = stable_delta
	})

	it('1/2 year later, Alice makes herself interest recipient again', async () => {
		const { time_error } = await this.network.timetravel({shift: '180d'})
		expect(time_error).to.be.undefined

		const new_stable_amount = Math.floor(this.deposit_tokens2 * (1 + this.interest_rate) ** 4)
		const stable_delta = new_stable_amount - this.deposit_stable_tokens
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		this.deposit_stable_tokens = new_stable_amount
		this.charlie_interest = stable_delta
	})

	it('Bob sends the stablecoins to Alice', async () => {
		const { unit, error } = await this.bob.sendMulti({
			asset: this.asset_stable,
			amount: this.bob_interest,
	...
	it('Charlie sends the stablecoins to Alice', async () => {
		const { unit, error } = await this.charlie.sendMulti({
			asset: this.asset_stable,
			amount: this.charlie_interest,
			to_address: this.aliceAddress,
			spend_unconfirmed: 'all',
		})
		...
		expect(balance[this.asset_stable].stable).to.be.equal(this.deposit_stable_tokens)
	})

	it('1.5 years later, Alice exchanges stable tokens for token2', async () => {
		const { time_error } = await this.network.timetravel({shift: '540d'})
		expect(time_error).to.be.undefined
