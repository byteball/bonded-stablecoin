...
describe('Issue combined with deposit', function () {
	...
	it('After 1/2 year, Alice buys tokens and immediately sends token2 to open a deposit', async () => {
	//	const { time_error } = await this.network.timetravel({shift: '180d'})
		const { time_error } = await this.network.timetravel({to: this.startTime + 180*24*3600*1000})
		...
		expect(parseFloat(vars['p2'].toPrecision(13))).to.be.equal(this.p2)
	...
	it('Alice buys more of tokens2', async () => {
		this.target_p2 = 1/this.price * (1 + this.interest_rate)**0.5
		const tokens1 = 0
		const tokens2 = 0.1e2
		const { amount, fee, fee_percent } = this.buy(tokens1, tokens2)
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		this.id2 = id
		this.deposit_stable_tokens2 = stable_tokens
		this.deposit_tokens2_2 = tokens2

		this.reserve_to_deposit = amount
	})


	it('Two years later, Alice closes the deposit and exchanges stable tokens for token2, then for bytes', async () => {
	//	const { time_error } = await this.network.timetravel({shift: '720d'})
		const { time_error } = await this.network.timetravel({to: this.startTime + (180+720)*24*3600*1000})
		expect(time_error).to.be.undefined

		const old_reserve = this.reserve
		const old_supply2 = this.supply2

		this.target_p2 = 1/this.price * (1 + this.interest_rate)**2.5
		const tokens1 = 0
		const tokens2 = 0.1e2
		const { amount, fee, fee_percent, reward, reward_percent } = this.buy(tokens1, -tokens2)

		expect(this.reserve).to.be.eq(old_reserve - this.reserve_to_deposit)
		expect(this.supply2).to.be.eq(old_supply2 - this.deposit_tokens2_2)

		this.supply -= this.deposit_stable_tokens2

		const { unit, error } = await this.alice.sendMulti({
			asset: this.asset_stable,
			base_outputs: [{ address: this.deposit_aa, amount: 1e4 }],
			asset_outputs: [{ address: this.deposit_aa, amount: this.deposit_stable_tokens2 }],
			messages: [{
				app: 'data',
				payload: {
					id: this.id2,
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		const { response: response2 } = await this.network.getAaResponseToUnit(response.response_unit)
		console.log('response2', response2.response.responseVars)
		expect(response2.response.error).to.be.undefined
		expect(response2.bounced).to.be.false
		expect(response2.response_unit).to.be.validUnit
		expect(response2.response.responseVars['reward%']).to.be.equal(reward_percent+'%')
		expect(response2.response.responseVars['fee%']).to.be.undefined

		const { vars: dvars } = await this.alice.readAAStateVars(this.deposit_aa)
		console.log(dvars)
		expect(dvars['deposit_' + this.id2]).to.be.undefined
		expect(dvars['supply']).to.be.equal(this.supply)
		...
		expect(Utils.getExternalPayments(unitObj)).to.equalInAnyOrder([{
			address: this.aliceAddress,
			amount: this.reserve_to_deposit + reward + 2000 - 1000, // 2000 AA-to-AA, 1000 network fee
		}])

	})

	it('Alice sends all to herself', async () => {
		const { unit, error } = await this.alice.sendMulti({
			asset: null,
			send_all: true,
			to_address: this.aliceAddress,
			spend_unconfirmed: 'all',
		})
