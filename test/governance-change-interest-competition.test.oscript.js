...
describe('Governance change interest rate with competition between two values', function () {
	...
	it('Half a year later, Alice votes for increase of interest to 20%', async () => {
		const { time_error } = await this.network.timetravel({shift: '180d'})
		expect(time_error).to.be.undefined

		const tokens1 = Math.floor(this.supply1 * 0.2)
		const name = 'interest_rate'
		const value = 0.2
	...
	it('Alice sends token1 to Bob', async () => {
		const { unit, error } = await this.alice.sendMulti({
			asset: this.asset1,
			amount: Math.round(this.supply1 * 0.2),
			to_address: this.bobAddress,
			spend_unconfirmed: 'all',
		})
	...
	it('1 day later, Bob votes for increase of interest to 30%', async () => {
		const { time_error } = await this.network.timetravel({shift: '1d'})
		expect(time_error).to.be.undefined

		const tokens1 = Math.floor(this.supply1 * 0.1)
		const name = 'interest_rate'
		const value = 0.3
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		console.log(vars)
		expect(vars['support_' + name + '_' + value]).to.be.equal(tokens1)
		expect(vars['support_' + name + '_' + value + '_' + this.bobAddress]).to.be.equal(tokens1)
		expect(vars['leader_' + name]).to.be.equal(0.2)
		expect(vars['balance_' + this.bobAddress]).to.be.equal(tokens1)

		const { unitObj } = await this.bob.getUnitInfo({ unit })
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		expect(vars['challenging_period_start_ts_' + name]).to.be.equal(this.ts)

	})

	it('Charlie supports Bob and also votes for increase of interest to 30%', async () => {
		const tokens1 = Math.floor(this.supply1 * 0.1)
		const name = 'interest_rate'
		const value = 0.3
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		console.log(vars)
		expect(vars['support_' + name + '_' + value]).to.be.equal(2*tokens1)
	...
	it('Bob votes again for increase of interest to 30% and his value becomes a leader', async () => {
		const tokens1 = Math.floor(this.supply1 * 0.01)
		const name = 'interest_rate'
		const value = 0.3
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		const { vars } = await this.bob.readAAStateVars(this.governance_aa)
		console.log(vars)
		expect(vars['support_' + name + '_' + value]).to.be.equal(this.supply1 * 0.21)
		expect(vars['support_' + name + '_' + value + '_' + this.bobAddress]).to.be.equal(this.supply1 * 0.11)
		expect(vars['leader_' + name]).to.be.equal(value)
		expect(vars['balance_' + this.bobAddress]).to.be.equal(this.supply1 * 0.11)
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		const { vars } = await this.bob.readAAStateVars(this.governance_aa)
		console.log(vars)
		expect(vars['support_' + this.name + '_' + '0.2']).to.be.equal(this.tokens1)
		expect(vars['support_' + this.name + '_' + this.value]).to.be.equal(this.supply1 * 0.21)
		expect(vars['support_' + this.name + '_' + '0.2' + '_' + this.aliceAddress]).to.be.equal(this.tokens1)
		expect(vars['support_' + this.name + '_' + this.value + '_' + this.charlieAddress]).to.be.equal(this.supply1 * 0.1)
		expect(vars['support_' + this.name + '_' + this.value + '_' + this.bobAddress]).to.be.equal(this.supply1 * 0.11)
		...
		expect(vars['balance_' + this.bobAddress]).to.be.equal(this.supply1 * 0.11)
		expect(vars['balance_' + this.charlieAddress]).to.be.equal(this.supply1 * 0.1)
		...
		expect(cvars['rate_update_ts']).to.be.equal(unitObj.timestamp)
		expect(cvars['growth_factor'].toPrecision(8)).to.be.equal((1.1**(186/360)).toPrecision(8))
	})
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.response.error).to.be.equal("support for interest_rate not removed yet")
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null

	})

	it('Alice unties her vote successfully without waiting', async () => {
		const { unit, error } = await this.alice.triggerAaWithData({
			toAddress: this.governance_aa,
			amount: 1e4,
			spend_unconfirmed: 'all',
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		const { vars } = await this.alice.readAAStateVars(this.governance_aa)
		console.log(vars)
		expect(vars['support_' + this.name + '_0.2']).to.be.equal(0)
		expect(vars['support_' + this.name + '_' + this.value]).to.be.equal(this.supply1 * 0.21)
	...
	it('Charlie tries to untie his vote too early but fails', async () => {
	...
	it('Charlie waits for 30 days and unties his vote successfully', async () => {
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		const { vars } = await this.charlie.readAAStateVars(this.governance_aa)
		console.log(vars)
		expect(vars['support_' + this.name + '_' + this.value]).to.be.equal(this.supply1 * 0.11)
		expect(vars['support_' + this.name + '_' + this.value + '_' + this.charlieAddress]).to.be.undefined
		expect(vars['leader_' + this.name]).to.be.equal(this.value)
		expect(vars['balance_' + this.charlieAddress]).to.be.equal(this.supply1 * 0.1)
		expect(vars[this.name]).to.be.equal(this.value)
	...
	it('Charlie withdraws successfully', async () => {
		const { unit, error } = await this.charlie.triggerAaWithData({
			toAddress: this.governance_aa,
			amount: 1e4,
			spend_unconfirmed: 'all',
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(vars['support_' + this.name + '_' + this.value + '_' + this.charlieAddress]).to.be.undefined
		expect(vars['leader_' + this.name]).to.be.equal(this.value)
		expect(vars['balance_' + this.charlieAddress]).to.be.equal(0)
		expect(vars[this.name]).to.be.equal(this.value)
		...
		expect(Utils.getExternalPayments(unitObj)).to.deep.equalInAnyOrder([{
			...
			asset: this.asset1,
			amount: this.supply1 * 0.1,
		}])

	})

	it('Bob unties his vote successfully', async () => {
		const { unit, error } = await this.bob.triggerAaWithData({
			toAddress: this.governance_aa,
			amount: 1e4,
			spend_unconfirmed: 'all',
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(vars['support_' + this.name + '_' + this.value + '_' + this.bobAddress]).to.be.undefined
		expect(vars['leader_' + this.name]).to.be.equal(this.value)
		expect(vars['balance_' + this.bobAddress]).to.be.equal(this.supply1 * 0.11)
		expect(vars[this.name]).to.be.equal(this.value)
	...
	it('Bob withdraws successfully', async () => {
		const { unit, error } = await this.bob.triggerAaWithData({
			toAddress: this.governance_aa,
			amount: 1e4,
			spend_unconfirmed: 'all',
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(vars['support_' + this.name + '_' + this.value + '_' + this.bobAddress]).to.be.undefined
		expect(vars['leader_' + this.name]).to.be.equal(this.value)
		expect(vars['balance_' + this.bobAddress]).to.be.equal(0)
		expect(vars[this.name]).to.be.equal(this.value)
	...
	it('1 year later, Alice buys more of the tokens2', async () => {
		...
		expect(time_error).to.be.undefined
		this.target_p2 = 1/this.price * 1.1**((180+5+1)/360) * 1.3**((30+360)/360)
