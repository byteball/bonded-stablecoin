...
describe('Deposits with interest recipient', function () {
	...
	it('Half a year later, Alice exchanges tokens2 for stable tokens with Bob set as interest recipient', async () => {
	...
	it('Two years later, Charlie withdraws interest to Bob', async () => {
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		this.supply += stable_delta
	})

	it('Alice mints additional stable tokens', async () => {
		const tokens2 = Math.floor(this.supply2 / 4)
		const stable_tokens = Math.floor(tokens2 * (1 + this.interest_rate)**2.5)
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		const { vars } = await this.alice.readAAStateVars(this.deposit_aa)
		console.log(vars)
		expect(vars['supply']).to.be.equal(this.deposit_stable_tokens + stable_tokens)
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		expect(vars['deposit_' + id2]).to.deep.equalInAnyOrder({
			amount: tokens2,
			stable_amount: stable_tokens,
			owner: this.aliceAddress,
			ts: unitObj.timestamp,
