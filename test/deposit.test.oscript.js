...
describe('Deposits', function () {
	...
	it('Two years later, Alice tries to exchange stable tokens for token2 but underpays', async () => {
		...
		const { unit, error } = await this.alice.sendMulti({
			asset: this.asset_stable,
			base_outputs: [{ address: this.deposit_aa, amount: 1e4 }],
			asset_outputs: [{ address: this.deposit_aa, amount: this.deposit_stable_tokens - 1 }],
			messages: [{
				app: 'data',
				payload: {
					id: this.id,
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.response.error).to.be.equal('expected ' + this.deposit_stable_tokens)
	...
	it('Alice successfully exchanges stable tokens for token2', async () => {
		const { unit, error } = await this.alice.sendMulti({
			asset: this.asset_stable,
			base_outputs: [{ address: this.deposit_aa, amount: 1e4 }],
