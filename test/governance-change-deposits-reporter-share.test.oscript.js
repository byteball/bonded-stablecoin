...
describe('Governance change deposits.reporter_share', function () {
	...
	it('Bob defines a new stablecoin', async () => {
		...
		// deposit params
		this.min_deposit_term = 600
		this.reporter_share = 0.3
		this.challenge_immunity_period = 1800
		this.challenging_period = 12 * 3600 // the default
		...
		const { unit, error } = await this.bob.triggerAaWithData({
			...
			data: {
				...
				'deposits.challenge_immunity_period': this.challenge_immunity_period,
			},
		})
	...
	it('Half a year later, Alice votes for increase of fee reporter_share to 0.4', async () => {
		...
		const name = 'deposits.reporter_share'
		const value = 0.4
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		const { vars: cvars } = await this.alice.readAAStateVars(this.curve_aa)
		console.log(cvars)
		expect(cvars['deposit_params']).to.be.undefined

		const { result, error: getterError } = await this.alice.executeGetter({
			aaAddress: this.deposit_aa,
			getter: 'get_deposit_params',
			args: []
		})
		expect(getterError).to.be.null
		expect(result).to.deep.equalInAnyOrder({
			min_deposit_term: this.min_deposit_term,
			challenging_period: this.challenging_period,
			challenge_immunity_period: this.challenge_immunity_period,
			reporter_share: this.reporter_share,
		})
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		this.reporter_share = this.value

		const { unitObj } = await this.bob.getUnitInfo({ unit: response.response_unit })

		const { vars: cvars } = await this.bob.readAAStateVars(this.curve_aa)
		console.log(cvars)
		expect(cvars['deposit_params']).to.deep.equalInAnyOrder({ reporter_share: this.value })
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.response.error).to.be.equal("support for deposits.reporter_share not removed yet")
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
