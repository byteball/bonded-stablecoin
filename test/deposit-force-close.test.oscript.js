...
describe('Deposits with force close', function () {
	...
	it('Bob adds protection to his deposit', async () => {
		const amount = 5e9 // note that Bob's deposit is 2 times larger than Alice's and this protection is equivalent to 2.5e9 protection on Alice's deposit
	...
	it('Alice tries to withdraw more protection than she has', async () => {
		const amount = this.protection + 1
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.response.error).to.be.equal("trying to withdraw more than you have: " + this.protection)
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
	})

	it('Alice withdraws part of protection', async () => {
		const amount = 1e9
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		this.protection_withdrawal_ts = unitObj.timestamp
		...
		expect(vars['deposit_' + this.id]).to.deep.equalInAnyOrder({
			...
			owner: this.aliceAddress,
			ts: this.ts,
			protection_withdrawal_ts: this.protection_withdrawal_ts,
		})
	})

	it("1 year later, Bob requests a force-close of the Alices's deposit", async () => {
	...
	it("Alice tries to close her deposit but fails", async () => {
		const { unit, error } = await this.alice.sendMulti({
			asset: this.asset_stable,
			base_outputs: [{ address: this.deposit_aa, amount: 1e4 }],
		...
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.validUnit
		expect(response.response.error).to.be.equal("force-close already requested")
		await this.network.witnessUntilStable(response.response_unit)
		...
		expect(Utils.getExternalPayments(unitObj)).to.deep.equalInAnyOrder([
			{
				asset: this.asset_stable,
				address: this.aliceAddress,
				amount: this.deposit_stable_tokens,
			},
		])
	})

	it("Alice tries to withdraw interest but fails", async () => {
		const { unit, error } = await this.alice.triggerAaWithData({
			toAddress: this.deposit_aa,
			amount: 1e4,
			data: {
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
		expect(response.response.error).to.be.equal("force-close requested, can't pay interest")

	})

	it("Alice tries to change interest recipient but fails", async () => {
		const { unit, error } = await this.alice.triggerAaWithData({
			toAddress: this.deposit_aa,
			amount: 1e4,
			data: {
				id: this.id,
				change_interest_recipient: 1,
				interest_recipient: this.bobAddress,
			},
			spend_unconfirmed: 'all',
		})
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
		expect(response.response.error).to.be.equal("force-close requested, can't change interest recipient")

	})

	it("Charlie tries to challenge the close indicating Bob's deposit but fails", async () => {
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
		expect(response.response.error).to.be.equal("the weaker deposit does not appear to be weaker: " + this.bob_protection_ratio)
	})

	it("Charlie tries to commit the close too early but fails", async () => {
		const { unit, error } = await this.charlie.triggerAaWithData({
			toAddress: this.deposit_aa,
			amount: 1e4,
			data: {
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.bounced).to.be.true
		expect(response.response_unit).to.be.null
		expect(response.response.error).to.be.equal("challenging period has not expired yet")

	})

	it("Charlie waits and commits the close", async () => {
		...
		this.supply -= this.deposit_stable_tokens
		...
		expect(vars['supply']).to.be.equal(this.supply)
		expect(vars['last_force_closed_protection_ratio']).to.be.equal(this.protection_ratio)
