...
describe('Deposits with non-base reserve asset, force close, and sending interest to an AA', function () {
	...
	before(async () => {
		this.network = await Network.create()
			...
			.with.wallet({ alice: {base: 1e6, reserve_asset: 1000e9} })
			.with.wallet({ bob: {base: 1e6, reserve_asset: 1000e9} })
			.with.wallet({ charlie: 1e6 })
			.with.explorer()
			.run()
		console.log('--- agents\n', this.network.agent)
	//	console.log('--- wallets\n', this.network.wallet)
	...
	//	this.explorer = await this.network.newObyteExplorer().ready()
		...
		this.reserve_asset = this.network.asset.reserve_asset

		const balance = await this.bob.getBalance()
		console.log(balance)
		expect(balance[this.reserve_asset].stable).to.be.equal(1000e9)
	})
	...
	it('Alice buys tokens', async () => {
		...
		const { unit, error } = await this.alice.sendMulti({
			...
			asset_outputs: [{ address: this.curve_aa, amount: amount }],
			messages: [{
				app: 'data',
				payload: {
					tokens1: tokens1,
	...
	it('Half a year later, Alice exchanges tokens2 for stable tokens', async () => {
		...
		const { unit, error } = await this.alice.sendMulti({
			...
			messages: [{
				app: 'data',
				payload: {
					interest_recipient: this.eater_aa
				}
			}],
	...
	it('Bob buys tokens and immediately sends token2 to open a deposit', async () => {
		...
		const { unit, error } = await this.bob.sendMulti({
			asset: this.reserve_asset,
			base_outputs: [{ address: this.curve_aa, amount: 1e4 }],
			asset_outputs: [{ address: this.curve_aa, amount: amount + fee }],
			messages: [{
				app: 'data',
				payload: {
					tokens1: tokens1,
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		...
		this.lost_peg_ts = unitObj.timestamp
	...
	it('Alice adds protection to her deposit', async () => {
		...
		const { unit, error } = await this.alice.sendMulti({
			asset: this.reserve_asset,
			base_outputs: [{ address: this.deposit_aa, amount: 1e4 }],
			asset_outputs: [{ address: this.deposit_aa, amount: amount }],
			messages: [{
				app: 'data',
				payload: {
					add_protection: 1,
	...
	it("1 year later, Bob requests a force-close of the Alices's deposit", async () => {
		...
		const { unit, error } = await this.bob.sendMulti({
			asset: this.asset_stable,
			base_outputs: [{ address: this.deposit_aa, amount: 1e4 }],
			asset_outputs: [{ address: this.deposit_aa, amount: new_stable_amount }],
			messages: [{
				app: 'data',
				payload: {
					id: this.id,
	...
	it("Charlie waits and commits the close", async () => {
		...
		const { vars: bvars } = await this.bob.readAAStateVars(this.bank_aa)
		console.log(bvars)
		expect(bvars['balance_' + this.eater_aa + '_' + this.asset_stable]).to.be.equal(this.interest)
	...
	it('Alice redeems part of the tokens1', async () => {
		this.target_p2 = 1/this.price * (1 + this.interest_rate)**((180+360+30)/360)
		const tokens1 = 0.1e9
		const tokens2 = 0
		...
		const { unit, error } = await this.alice.sendMulti({
			asset: this.asset1,
			base_outputs: [{ address: this.curve_aa, amount: 1e4 }],
			asset_outputs: [{ address: this.curve_aa, amount: tokens1 }],
		})
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.bounced).to.be.false
		expect(response.response_unit).to.be.validUnit
		expect(response.response.responseVars['reward%']).to.be.equal(reward_percent+'%')
