...
describe('Issue redeem before the oracle posts for the first time', function () {
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		this.buy = (tokens1, tokens2) => {
			...
			const old_distance = (this.reserve && this.target_p2) ? Math.abs(this.p2 - this.target_p2) / this.target_p2 : 0
			const new_distance = this.target_p2 ? Math.abs(p2 - this.target_p2) / this.target_p2 : 0
			let fee = this.getFee(avg_reserve, old_distance, new_distance);
			if (fee > 0) {
				const reverse_reward = Math.floor((1 - this.distance / new_distance) * this.fast_capacity); // rough approximation
			}

			const fee_percent = round(fee / abs_reserve_delta * 100, 4)
			const reward = this.distance ? Math.floor((1 - new_distance / this.distance) * this.fast_capacity) : 0;
			const reward_percent = round(reward / abs_reserve_delta * 100, 4)

			console.log('p2 =', p2, 'target p2 =', this.target_p2, 'amount =', amount, 'fee =', fee, 'reward =', reward, 'old distance =', old_distance, 'new distance =', new_distance)
	...
	it('Alice redeems part of the tokens2', async () => {
		const tokens1 = 0
		const tokens2 = 0.5e2
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.bounced).to.be.false
		expect(response.response_unit).to.be.validUnit
		expect(response.response.responseVars['reward%']).to.be.undefined
	...
	//	console.log(JSON.stringify(unitObj, null, '\t'))
		expect(Utils.getExternalPayments(unitObj)).to.deep.equalInAnyOrder([{
			address: this.aliceAddress,
			amount: -amount + reward + 10000 - 1000,
		}])
	...
	it('Move funds between slow and fast capacitor', async () => {
		...
		this.fast_capacity += amount_moved
